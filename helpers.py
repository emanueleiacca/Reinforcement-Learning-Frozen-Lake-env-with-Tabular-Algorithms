...
class FrozenLakeDistanceRewardWrapper(gym.Wrapper):
    def __init__(self, env):
        super().__init__(env)
        self.env = env
    ...
    def step(self, action):
        ...
        if reward == 0 and done:
            ...
            rows = columns = self.env.nrow
            state_row = next_state // (rows - 1)
            state_column =  next_state % (columns - 1)
            distance = rows - 1 - state_row + columns - 1 - state_column
...
class DiscretizedObservationWrapper(gym.ObservationWrapper):
    def __init__(self, env, n_bins=10, low=None, high=None):
        ...
        self.observation_space = Discrete((n_bins+1) ** len(low))
